...
export type Student={id:string;workshop_id:string;name:string;email:string;national_id:string;revision:number;updated_at:string;[key:string]:string|number|null};
...
export function maskNationalId(value:string){return value.length<5?'***':`${value.slice(0,3)}${'*'.repeat(Math.max(0,value.length-5))}${value.slice(-2)}`;}
