...
export type StationDefinition={key:string;title:string;testDate:string;complaint:string;diagnosis:string;prompt:string;domainMax?:DomainValues};
export const DEFAULT_STATIONS:StationDefinition[]=[
 {key:'q1',title:'',testDate:'',complaint:'',diagnosis:'',prompt:''},
];
...
export function workshopStations(workshop:Workshop|null|undefined):StationDefinition[]{const configured=workshop?.stations;if(!configured?.length)return DEFAULT_STATIONS;return configured.map((station,index)=>({key:station.key||`q${index+1}`,title:station.title||'',testDate:station.testDate||'',complaint:station.complaint||'',diagnosis:station.diagnosis||'',prompt:station.prompt||'',...(station.domainMax ? {domainMax:station.domainMax} : {})}));}
