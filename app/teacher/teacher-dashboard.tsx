import { domainFormValues, domainTotal, validateDomainMax, validateDomainScores } from '@/lib/domains';
  ...
  async function saveScoreBatch(form: HTMLFormElement) {
    ...
    try {
      const items = currentData.students.filter((student) => selectedStudentIds.has(student.id)).map((student) => ({
        ...
        ...(() => { const domains=domainFormValues(fields,`${student.id}_${selectedStation.key}_domain`); const score=domains ? domainTotal(validateDomainScores(domains,selectedStation.domainMax)) : null; return {...validateGrade(score,fields.get(`${student.id}_rating`)==='' ? null : Number(fields.get(`${student.id}_rating`))),domains}; })(),
