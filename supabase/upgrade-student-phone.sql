...
create unique index if not exists students_workshop_phone_unique on nptc_private.students(workshop_id,phone) where phone is not null;
...
begin
 ...
 select coalesce(jsonb_agg(jsonb_build_object(
  'id',s.id,'name',s.name,'phone',s.phone,'workshopName',w.name,'published',w.published,
...
-- 批次匯入時傳入 [{"name":"王小明","email":"student@example.com","phone":"0912345678"}]。
...
begin
 ...
 if action='bulkImportStudents' then
  ...
  for row_item in select value from jsonb_array_elements(body->'students') loop
   if jsonb_typeof(row_item->'name') is distinct from 'string' or jsonb_typeof(row_item->'email') is distinct from 'string' or jsonb_typeof(row_item->'phone') is distinct from 'string' then raise exception '匯入資料需包含手機電話、姓名與 Email。'; end if;
