...
function PasswordFields() {
  return <><label className="block">設定密碼（至少 8 個字元）<Input name="password" type="password" autoComplete="new-password" minLength={8} required /></label><label className="block">再次輸入密碼<Input name="confirm" type="password" autoComplete="new-password" minLength={8} required /></label></>;
}
...
export function StudentLogin() {
  ...
  return <section className="mx-auto max-w-4xl px-6 py-12">
    ...
    <form onSubmit={submit} className="mt-6 space-y-5 rounded-xl border bg-white p-6 sm:p-8">
      ...
      {mode === 'activate' && <><p className="text-sm">請填寫報名時的資料。系統會先驗證信箱，再核對管理員建立的名冊。</p><label className="block">姓名<Input name="name" autoComplete="name" maxLength={100} required /></label></>}
      <label className="block">Email<Input name="email" type="email" autoComplete="username" required /></label>
...
export function StudentActivation({ email, status, onComplete }: { email: string; status: Onboarding; onComplete: () => void }) {
  ...
  return <section className="mx-auto max-w-3xl px-6 py-12"><h1 className="text-2xl font-bold">完成首次啟用</h1><p className="my-4">{state.stage === 'unclaimed' ? 'Email 已驗證，請核對報名姓名與手機。' : '名冊已核對，請補齊個人資料並設定日後登入密碼。'}</p>
    <form onSubmit={submit} className="space-y-5 rounded-xl border bg-white p-6" key={state.stage}>
      <label className="block">Email（登入帳號）<Input value={email} readOnly /></label>
      {state.stage === 'unclaimed' ? <><label className="block">姓名<Input name="name" defaultValue={sessionStorage.getItem('nptc-activation-name') ?? ''} required /></label><label className="block">手機電話<Input name="phone" type="tel" pattern="09[0-9]{8}" defaultValue={sessionStorage.getItem('nptc-activation-phone') ?? ''} required /></label></> : <>
        <div className="grid gap-4 sm:grid-cols-2"><label>姓名<Input value={student?.name ?? ''} readOnly /></label><label>手機電話<Input value={student?.phone ?? ''} readOnly /></label></div><p className="text-sm">以上資料由管理員建立，如需更正請聯絡管理員。</p>
        <label className="block">護理年資（年）<Input name="nursingYears" type="number" min={0} max={60} step={1} defaultValue={student?.nursingYears ?? ''} required /></label>
        <HospitalPicker defaultValue={student?.hospital ?? ''} hospitals={hospitals} loading={loading} />
        <label className="block">服務單位<Input name="unit" maxLength={100} defaultValue={student?.unit ?? ''} required /></label>
