...
export function AnalysisView({ data,currentWorkshopId,onWorkshopChange,onRefresh }: { data: AnalysisData[]; currentWorkshopId?: string; onWorkshopChange?: (id:string)=>void; onRefresh?: ()=>void }) {
  ...
  useEffect(()=>{if(person){detailRef.current?.focus(); detailRef.current?.scrollIntoView({behavior:'smooth',block:'start'});}},[person]);
  ...
  const activePerson = person ? learners(allRows).find(p=>p.identity===person) : undefined;
  const history = person ? timeline(allRows,person) : [];
  ...
  return <div className="learning-analysis">
    ...
    <section className="analysis-panel" aria-label="學員逐題成績"><h2>學員逐題成績</h2><p>每格顯示該題分數、邊緣及格分數與判定；點選姓名可查看歷次紀錄。未評分或尚無門檻的題目不計入未達及格題數。</p><div className="analysis-table-scroll"><table className="analysis-student-exams"><thead><tr><th>學員</th>{stationRows.map(s=><th key={s.id} scope="col">{s.name}<small>{s.workshop} · {s.date}</small></th>)}<th>未達及格題數</th></tr></thead><tbody>{people.map(p=>{
      const studentExams = p.records.flatMap(r=>r.exams);
      return <tr key={p.identity}><th scope="row"><button className="analysis-link" aria-expanded={person===p.identity} aria-controls="analysis-person" onClick={e=>{opener.current=e.currentTarget;setPerson(p.identity);}}>{p.names}</button><small>{p.email || '未提供 Email，未跨梯次合併'}</small></th>
    ...
    {activePerson && <section id="analysis-person" ref={detailRef} tabIndex={-1} className="analysis-panel analysis-person" aria-label={`${activePerson.names}的歷次紀錄`}><button className="analysis-link" onClick={()=>{setPerson(null);opener.current?.focus();}}><ArrowLeft size={16} aria-hidden="true"/>關閉個人紀錄</button><h2>{activePerson.names}｜歷次學習紀錄</h2><p>{activePerson.email} · 全部 {new Set(activePerson.records.map(r=>r.workshop.id)).size} 個梯次，不受上方篩選限制。</p><p className="analysis-note">依相同 Email 串接；Email 變更將視為不同紀錄。同一 Email 如有不同姓名，請核對名冊。不同題目與評分標準的分數變化，不能直接視為進步或退步。</p><div className="analysis-table-scroll"><table><thead><tr><th>日期／梯次</th><th>題目</th><th>分數</th><th>Global Rating</th><th>判定</th><th>老師回饋</th></tr></thead><tbody>{history.map(e=><tr key={`${e.workshopId}:${e.key}`}><td>{e.date || '未設定日期'}<small>{e.workshopName}</small></td><th scope="row">{e.title}</th><td><Score value={e.score}/></td><td>{e.rating ?? '—'}</td><td>{examStatus(e)}<small>門檻 {number(e.threshold)}</small></td><td className="analysis-feedback">{e.feedback || '尚無回饋'}</td></tr>)}</tbody></table></div>{!history.length && <p className="analysis-empty">尚無測驗題目紀錄。</p>}</section>}
